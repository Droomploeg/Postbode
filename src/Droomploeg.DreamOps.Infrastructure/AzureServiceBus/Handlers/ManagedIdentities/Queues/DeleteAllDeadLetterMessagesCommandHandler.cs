...
public class DeleteAllDeadLetterMessagesCommandHandler<TSendMessage, TReceiveMessage> : ICommandHandler<DeleteAllDeadLetterMessagesCommand>
    where TReceiveMessage : notnull
{
    private readonly IDeadLetterQueueAdapter<TSendMessage, TReceiveMessage> _adapter;
    private readonly IWorkerService _service;

    public DeleteAllDeadLetterMessagesCommandHandler(
    ...
    public Task HandleAsync(DeleteAllDeadLetterMessagesCommand command, CancellationToken cancellationToken = default)
    {
        var entityName = command.QueueName;
        var workItem = new WorkerItem(
            entityName,
            $"Delete all dead-letter message from queue '{entityName}'",
